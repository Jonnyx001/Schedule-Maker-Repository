...
def generate_schedule(weeks, matchups, teamlist, time_limit, weeknum):
    games_in_week = len(teamlist) / 2
    ...
    while not gen_finished:
        if time.time() - start_time > time_limit:
            print(f"\nTime limit exceeded (20 seconds). Schedule generation failed.")
            time.sleep(4)
        ...
        for week in weeks:
            ...
            while len(matchups_this_week) < games_in_week:
                added = False
                for matchup in matchupscopy:
                    if matchup.usesleft <= 0:
                        continue
                    if matchup.hometeam in usedteams or matchup.awayteam in usedteams:
                        continue
                    matchups_this_week.append(matchup)
                    usedteams.extend([matchup.hometeam, matchup.awayteam])
                    matchup.usesleft -= 1
                    added = True
                    #time.sleep(0.05)
                    #print(f"Added: {matchup.hometeam} vs {matchup.awayteam} | Uses left: {matchup.usesleft}")
                    break
                if not added:
                    #time.sleep(0.05)
        ...
        for week in schedule:
            ...
            gen_finished = True
            #time.sleep(0.2)
            #print(f"{week.name} was added to schedule with {len(week.matchups)} matchups: {[f'{m.hometeam} vs {m.awayteam}' for m in week.matchups]}")
            print(f"\nSchedule generated on attempt {attempt_count} after {time.time() - start_time:.3f} seconds.")
            
    while not shuffle_finished:
        if time.time() - start_time > time_limit:
            print(f"\nTime limit exceeded (20 seconds). Schedule was generated but shuffling failed.")
            time.sleep(4)
            print("Closing the program...")
            time.sleep(2)
            exit(1)

        schedule = shuffle_so_no_repeats(schedule, weeknum)
...
def shuffle_so_no_repeats(schedule, weeknum):
    last_week = 0
    current_week = 1
    shuffle_count = 0
    ...
    while current_week < weeknum:
        ...
        for week in schedule[last_week].matchups:
            last_week_matchups.append([{week.hometeam}, {week.awayteam}])

        for week in schedule[current_week].matchups:
            current_week_matchups.append([{week.hometeam}, {week.awayteam}])

        for matchup in current_week_matchups:
            if matchup in last_week_matchups or swap_2_item_list(matchup) in last_week_matchups:
    ...
    print(f"Shuffling complete after {shuffle_count} shuffles.")
    return schedule
